...
#pragma pack(pop)
	...
	KLI_FORCEINLINE ULONG_PTR find_kernel_export()
	{
		...
		const auto export_directory = (detail::PIMAGE_EXPORT_DIRECTORY)(kernel_base +
			nt_headers->OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_EXPORT].VirtualAddress);

		const auto address_of_functions = (UINT32*)(kernel_base + export_directory->AddressOfFunctions);
		const auto address_of_names = (UINT32*)(kernel_base + export_directory->AddressOfNames);
		const auto address_of_name_ordinals = (UINT16*)(kernel_base + export_directory->AddressOfNameOrdinals);
		...
		for (UINT32 i = 0; i < export_directory->NumberOfNames; ++i)
		{
			...
			{
				return kernel_base + address_of_functions[address_of_name_ordinals[i]];
			}
		}
